...
async function loadLines(setSt){
  ...
  try{
    ...
    if(!matched.length)throw new Error(`No games matched. Got: ${evs.slice(0,3).map(e=>e.away_team+"@"+e.home_team).join(", ")}`);
    ...
    setSt(live?`✅ ${tot} props across ${live} games (DK priority)`:`⚠ Props not posted yet — DK posts ~2-3 hrs before first pitch`);
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Courier New',monospace",color:C.white,padding:14}}>
      ...
      <div style={{marginTop:12,color:C.muted,fontSize:8,textAlign:"center"}}>
        March 27 2026 · Lineups: MLB.com/RotoWire/ESPN confirmed · Stats: BR/FG/Savant 2025 · Park: Savant Statcast 3yr
      </div>
    </div>
  );
}
