...
const { buildBreakdown, MIN_N, MIN_HISTORY } = await import('../src/analysis/profitability.js');
...
async function grade(date) {
  ...
  const byKey = new Map(graded.map((r) => [keyOf(r), r]));
  let settled = 0;
  for (const row of day.rows) {
...
switch (cmd) {
  ...
  case 'report':
    ...
    break;
  default:
    console.log('usage: node tools/track.mjs [daily|snapshot|close|grade|report] [YYYY-MM-DD]');
}
